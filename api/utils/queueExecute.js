          ...
          .then(codeToBeExecuted => {
            ...
            try {
              request({
                ...
                json: true,
                body: { ...codeToBeExecuted.dataValues,
                  secretString
                }
              }, (err, response, body) => {
                if (!response) {
                ...
                let results, player1Score, player2Score, player2ExitStatus, player1ExitStatus, player1Dlog, player2Dlog, cancelMatch, runtimeErrorPresentP1, runtimeErrorPresentP2;
                ...
                if (response.body.success) {
                  ...
                  cancelMatch = player1ExitStatus === 'UNDEFINED' ||
                    player2ExitStatus === 'UNDEFINED' ||
                    player1ExitStatus === 'EXCEEDED_INSTRUCTION_LIMIT' ||
                    player2ExitStatus === 'EXCEEDED_INSTRUCTION_LIMIT';
									runtimeErrorPresentP1 = player1ExitStatus === 'RUNTIME_ERROR';
									runtimeErrorPresentP2 = player2ExitStatus === 'RUNTIME_ERROR';
                } else {
                  cancelMatch = true;
                  ...
                  .then(user1 => {
                    if (codeToBeExecuted.isAi) {
                      ...
                      models.User.findOne({
                        ...
                        .then(user2 => {
													if (err) console.log(err);
													if(runtimeErrorPresentP1 || runtimeErrorPresentP2){
														models.Match.update({
															status: 'error',
															error_log: body.error
														}, {
														...
														.then(match => {
															...
															if(runtimeErrorPresentP1){
																score1 = elo.updateRating(expec1, 0, score1);
																score2 = elo.updateRating(expec2, 1, score2);
																models.Notification.create({
																	type: 'ERROR',
																	title: 'Execution Error',
																	message: `Runtime error! You just lost a match against ${user2.name}. Please check your code`,
																	isRead: false,
																	user_id: userId
																})
																.then(notification => {
																	models.Notification.create({
																		...
																		message: `${user1.name}\'s code had a runtime error. You win!`,
																		isRead: false,
																		user_id: opponentId
																	})
																	.then(notification => {
																...
																models.Notification.create({
																	type: 'INFORMATION',
																	title: 'Execution Error',
																	message: `Runtime error! You just lost a match against ${user1.name}. Please check your code`,
																	isRead: false,
																	user_id: opponentId
																})
																.then(notification => {
																	models.Notification.create({
																		type: 'SUCCESS',
																		title: 'Opponent Error',
																		message: `${user2.name}\'s code had a runtime error. You win!`,
																		isRead: false,
																		user_id: userId
																	})
																	.then(notification => {
												...
												}else if (!body.success || cancelMatch) {
                            models.Match.update({
                                status: 'error',
                                error_log: body.error
                              }, {
